...
def oneline_wrapper(dic_agent_conf, dic_traffic_env_conf, dic_path, roadnet, trafficflow):
    print("\n===== ONELINE WRAPPER INITIALIZATION =====")
    print(f"Roadnet: {roadnet}")
    print(f"Traffic Flow: {trafficflow}")
    print(f"Configuration: {dic_traffic_env_conf['MODEL_NAME']}")
    print("===========================================\n")
    ...
    for i in range(1):
        ...
        if hasattr(oneline, 'env'):
            print("Environment created successfully")
            print(f"Environment type: {type(oneline.env).__name__}")
            
            if hasattr(oneline.env, 'list_intersection'):
                print(f"Number of intersections: {len(oneline.env.list_intersection)}")
                if len(oneline.env.list_intersection) > 0:
                    print(f"First intersection name: {oneline.env.list_intersection[0].inter_name}")
            
            if hasattr(oneline.env, 'intersection_dict'):
                print(f"Intersection dictionary contains {len(oneline.env.intersection_dict)} entries")
        else:
            print("Warning: No environment attribute found in oneline object")
        print("================================\n")
