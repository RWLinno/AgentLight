...
class OneLine:
    ...
    def train(self, round):
        ...
        while not done and current_time < total_run_cnt:
            ...
            # multi-thread for ChatGPT
            if "ChatGPT" in self.dic_traffic_env_conf["MODEL_NAME"]:
                # Start threads one at a time instead of all at once
                print(f"Starting {len(threads)} ChatGPT agent threads")
                for i, thread in enumerate(threads):
                    # Use direct access to env for each agent
                    self.agents[i].env = self.env  # Give agent direct reference to the environment
                    thread.start()
                    print(f"Started thread {i+1}/{len(threads)}")
            ...
            # multi-thread for open_llm with different thread handling
            elif "open_llm" in self.dic_traffic_env_conf["MODEL_NAME"]:
                started_thread_id = []
