...
def get_train_val_env(env_class, config: dict):
    ...
    elif config.env.name == 'cityflow':
        # Initialize CityFlow environment using env_kwargs from config
        env_kwargs = config.env.env_kwargs
        
        # Create training environment
        env = env_class(
            path_to_log=env_kwargs.get('path_to_log', './log'),
            path_to_work_directory=env_kwargs.get('path_to_work_directory', './LLMLight/data/Jinan/3_4'),
            dic_traffic_env_conf=env_kwargs.get('dic_traffic_env_conf', {
            ...
            dic_path=env_kwargs.get('dic_path', {"PATH_TO_DATA": "./data/Jinan/3_4"}),
            #max_steps=env_kwargs.get('max_steps', 3600),
            #um_intersections=env_kwargs.get('num_intersections', 12)
        )
        
        # Create validation environment (same as training for now)
        val_env = env_class(
            path_to_log=env_kwargs.get('path_to_log', './log'),
            path_to_work_directory=env_kwargs.get('path_to_work_directory', './LLMLight/data/Jinan/3_4'),
            dic_traffic_env_conf=env_kwargs.get('dic_traffic_env_conf', {
            ...
            dic_path=env_kwargs.get('dic_path', {"PATH_TO_DATA": "./data/Jinan/3_4"}),
            #max_steps=env_kwargs.get('max_steps', 3600),
            #num_intersections=env_kwargs.get('num_intersections', 12)
        )
    else:
